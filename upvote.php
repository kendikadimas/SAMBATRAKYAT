...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['laporan_id'])) {
    ...
    // Tambahkan 1 pada kolom `upvotes` berdasarkan ID laporan
    $stmt = $db->prepare("UPDATE laporan SET upvotes = upvotes + 1 WHERE id = ?");
    if ($stmt->execute([$laporan_id])) {
        ...
        echo json_encode(['success' => false, 'message' => 'Gagal menambah upvote']);
    }
}
?>
